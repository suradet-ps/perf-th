...
if ([string]::IsNullOrWhiteSpace($Orig)) {
    $candidates = @(
        (Join-Path $PSScriptRoot '..\..\perf-book\src'),
        (Join-Path $PSScriptRoot '..\perf-book\src'),
        (Join-Path (Get-Location) 'perf-book\src')
    )
...
if ([string]::IsNullOrWhiteSpace($Orig) -or (-not (Test-Path -LiteralPath $Orig))) {
    Write-Error "Cannot find upstream perf-book src directory.`nPlease provide the path using: ./scripts/verify-translation.ps1 -Orig <path-to-perf-book/src>"
    exit 1
}
...
function Resolve-LinkTarget($url, $rel) {
    ...
    # Resolve local link targets (including root-absolute ones like /perf-book/introduction.md)
